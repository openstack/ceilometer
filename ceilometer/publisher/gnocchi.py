...
class GnocchiPublisher(publisher.ConfigPublisherBase):
    ...
    To disable filtering the Gnocchi project, use the following option:
    ...
    def __init__(self, conf, parsed_url):
        ...
        self.enable_filter_project = strutils.bool_from_string(
            options.get('enable_filter_project', ['true'])[-1])
        if self.enable_filter_project:
            self.filter_project = options.get('filter_project',
                                              ['service'])[-1]
            self.filter_domain = options.get('filter_domain', ['Default'])[-1]
        else:
            LOG.debug(
                "Filtering Gnocchi project is disabled, "
    ...
    def gnocchi_project_id(self):
        if not self.enable_filter_project:
            return None
    ...
    def filter_gnocchi_activity_openstack(self, samples):
        ...
        """
        if not self.enable_filter_project:
            LOG.debug("Filtering Gnocchi activities is disabled, "
                      "samples [%s] will not be filtered.", samples)
            return samples
        filtered_samples = []
